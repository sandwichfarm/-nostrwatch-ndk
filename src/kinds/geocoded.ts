...
import type { NDKFilter, NDKTag, NostrEvent  } from "@nostr-dev-kit/ndk";
...
export type EventGeoCodedCallback = (events: Set<EventGeoCoded>) => Promise<Set<EventGeoCoded>>;
...
const eventGeoCodedCallbackDefault: EventGeoCodedCallback = async (evs: Set<EventGeoCoded>)=>evs; 
...
export class EventGeoCoded extends NDKEvent {
    ...
    constructor( ndk: NDK | undefined, rawEvent?: NostrEvent ) {
        super(ndk, rawEvent);
        if(EventGeoCoded.isGeohash(this.geohash)){
    ...
    /**
     * @description Compares two geohashes and sorts them based on their length in descending order.
     * 
    ...
    /**
     * @description Generates a list of filterable geohashes from a given full geohash.
    ...
    /**
     * @description Sets the geographical coordinates or geohash for the current instance, updating latitude, longitude, and geohash values accordingly.
    ...
    /**
     * @description Gets the current geographical coordinates (latitude and longitude) prioritizing direct coordinates if available.
    ...
    /**
     * @description Sets the primary geohash for the current instance and updates the list of geohashes with varying precision based on this value.
    ...
    set geohash(value: Geohash) {
        if(!EventGeoCoded.isGeohash(value)) return;
        const geohashes = 
            EventGeoCoded
                .generateFilterableGeohash(String(value))
                .sort( EventGeoCoded.sortGeohashesFn );
        this._updateGeohashTags(geohashes);
    }

    /**
     * @description Sets the list of geohashes for the current instance, ensuring no duplicates and updating related class properties accordingly.
    ...
    set geohashes(values: Geohash[]) {
        values = values.filter(EventGeoCoded.isGeohash);
        this._removeGeoHashes();
        this._updateGeohashTags(Array.from(new Set<string>(values)));
    }

    /**
     * @description Gets the current list of geohashes associated with the instance, sorted by descending precision.
     * 
    ...
    get geohashes(): Geohash[] | undefined {
        const tags = 
            this.tags
                .filter( EventGeoCoded.geohashFilterFn )
    ...
    /**
     * @description Gets the the most precise geohash for the current instance, which is the first geohash in the list of geohashes sorted by descending precision.
     * 
    ...
    /**
     * @description Retrieves all geo tags from the event's tags.
     * 
    ...
    /**
     * @description Fetches events and sorts by distance with a given geohash
     * 
    ...
    public static async fetchNearby(
        ...
        try {
            ...
            if (events.size) {
                events = new Set([...EventGeoCoded.sortGeospatial(geohash, events)]);
            }
    ...
    /**
     * @description Sorts an array of `EventGeoCoded` instances based on their distance from a given latitude and longitude.
     * 
    ...
    public static sortGeospatial = (
        coords: Coords, 
        geoCodedEvents: Set<EventGeoCoded>, 
        asc: boolean = true
    ): Set<EventGeoCoded> => {
    ...
    /**
     * @description Encodes latitude and longitude into a geohash string.
     * 
    ...
    public static encodeGeohash(coords: Coords, precision: number = this.GEOHASH_PRECISION): string {
        ...
        while (geohash.length < precision) {
            ...
            if (bit < 4) {
                ...
                geohash += EventGeoCoded.BASE32.charAt(ch);
                bit = 0;
                ch = 0;
            }
        }
    ...
    /**
     * @description Decodes a geohash string into its latitude and longitude representation.
     * 
    ...
    public static decodeGeohash(hashString: string): DD {
        ...
        for (let i = 0; i < hashString.length; i++) {
            ...
            const charIndex = EventGeoCoded.BASE32.indexOf(char);
            for (let j = 0; j < 5; j++) {
                const mask = 1 << (4 - j);
                if (isEven) {
    ...
    /**
     * @description Checks if a string is a valid geohash.
     * 
    ...
    public static isGeohash(str: string | undefined): boolean {
        ...
        for (let i = 0; i < str.length; i++) {
            if (EventGeoCoded.BASE32.indexOf(str.charAt(i)) === -1) {
                return false;
            }
        }
        return true;
    ...
    /**
     * @description Calculates the great-circle distance between two points on the Earth's surface given their latitudes and longitudes.
    ...
    /**
     * @description Parses a Coords object, potentially a geohash, into a DD object
     * 
     * @param {Coords} coords 
     * @returns A DD object
     */
    ...
    /**
     * @description Refines the search interval for a geohash decoding process based on a character from the geohash.
    ...
    /**
     * @description Removes tags by their value (the value at position 1 in the tag array).
     * 
    ...
    /**
     * @description Removes tags by marker (the value at last position in the tag array).
     * 
    ...
    /**
     * @description Helper method to find the first tag value by its marker.
     * 
    ...
    /**
     * @description Retrieves tags that are indexed, identified by having their first element's length equal to 1.
     * 
    ...
    /**
     * @description Adds a geo tags to the event's tags, updating or removing existing tags as necessary.
    ...
    /**
     * @description Removes geohash tags ('gh') from the event's tags, cleaning up the tag array from geohash (gh) tags.
    ...
    private _removeGeoHashes(): void {
        ...
        this.tags = this.tags.filter( tag => !(EventGeoCoded.geohashFilterFn(tag)) ); 
    }

    /**
     * @description Updates the geohashes and corresponding tags for this instance.
     * 
