...
import { EventGeoCoded } from './geocoded';
...
describe('EventGeoCoded', () => {
    ...
    let eventGeoCodedNip23: EventGeoCoded;

    beforeEach(() => {
        ndk = new NDK();
        eventGeoCoded = new EventGeoCoded(ndk, rawEvent);
        eventGeoCoded2 = new EventGeoCoded(ndk, geohashEvent);
        eventGeoCodedNip23 = new EventGeoCoded(ndk, geohashEventNip23);
    });
    ...
    describe('@static', () => {
        ...
        describe('isGeohash', () => {
            it('should correctly identify a geohash', () => {
                expect(EventGeoCoded.isGeohash('u0yjjd6j5sff')).toBeTruthy();
            });

            it('should correctly identify a non-geohash', () => {
                expect(EventGeoCoded.isGeohash('aiol')).toBeFalsy();
                expect(EventGeoCoded.isGeohash('a')).toBeFalsy();
                expect(EventGeoCoded.isGeohash('i')).toBeFalsy();
                expect(EventGeoCoded.isGeohash('o')).toBeFalsy();
                expect(EventGeoCoded.isGeohash('l')).toBeFalsy();
            });
        });
        ...
        describe('sortGeohashesFn', () => {
            it('should sort geohashes by length in descending order', () => {
                const sorted = ['u', 'u0yjjd6j5s', 'u0yjjd6j5sf', 'u0yjjd6j5sff'].sort(EventGeoCoded.sortGeohashesFn);
        ...
        describe('fetchNearby', () => {
            ...
            beforeEach( () => {
                vi.spyOn(ndk, 'fetchEvents').mockImplementation(async (filter) => {
                    return new Set([
                        eventGeoCoded2,
            ...
            describe('fetchNearby with options', () => {
                it('should fetch events nearby a given geohash and use options', async () => {
                    ...
                    const nearbyEvents = await EventGeoCoded.fetchNearby(ndk, 'u0yjjd6j5sff', undefined, options);
                    expect(nearbyEvents).toBeInstanceOf(Set);
                    expect(nearbyEvents.size).toBeGreaterThan(0);
                    expect(Array.from(nearbyEvents)?.[0].geohash).toEqual('u0yjjd6j5sff');
                });
                ...
                it('should return no events when callback is used to clear all events', async () => {
                    const options: FetchNearbyRelayOptions = { 
                        callbackFilter: async (evs: Set<EventGeoCoded>) => new Set()
                    };
                    const nearbyEvents = await EventGeoCoded.fetchNearby(ndk, 'u0yjjd6j5sff', undefined, options);
                    expect(nearbyEvents).toBeInstanceOf(Set);
